...
int main() {
  ...
  // soluçao:
  for(int i = 1; i < 100*4; i= i+4) {
    int media = (pchar[i]+pchar[i+1]+pchar[i+2])/3;
    pchar[i+0] = media; // azul
    pchar[i+1] = media; // verde
    pchar[i+2] = media; // vermelho
  }
